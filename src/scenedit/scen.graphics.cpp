...
void run_startup_g() {
	sf::Event event;
	auto & pict_to_draw = *ResMgr::graphics.get("edsplash", true);
	...
	if(cur_viewing_mode == 0) {
		...
		for(short q = 0; q < 9; q++)
			for(short r = 0; r < 9; r++) {
				...
				auto const & fields_gworld = *ResMgr::graphics.get("fields");
				auto const & vehicle_gworld = *ResMgr::graphics.get("vehicle");
...
void draw_one_tiny_terrain_spot (short i,short j,ter_num_t terrain_to_draw,short size,bool road) {
	...
	if(drawLargeIcon) {
		...
		if(picture_wanted >= 1000) {
			...
			auto const & small_ter_gworld = *ResMgr::graphics.get("termap");
...
static void place_selected_terrain(ter_num_t ter, rectangle draw_rect) {
	...
	if(picture_wanted >= 1000)	{
		...
		int which_sheet = picture_wanted / 50;
		auto const & terrain_gworld = *ResMgr::graphics.get("ter" + std::to_string(1 + which_sheet));
