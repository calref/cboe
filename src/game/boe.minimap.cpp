				...
				if(expl != 0) {
					...
					if(pic >= 1000) {
						...
						if(pic >= 960) {
							...
							rect_draw_some_item(*ResMgr::graphics.get("teranim"), custom_from, gworld, draw_rect);
						} else {
							int which_sheet = pic / 50;
							auto const &src_gw = *ResMgr::graphics.get("ter" + std::to_string(1 + which_sheet));
							pic %= 50;
					...
					if(is_out() ? univ.out->roads[where.x][where.y] : univ.town.is_road(where.x,where.y)) {
						draw_rect.inset(1,1);
						rect_draw_some_item(*ResMgr::graphics.get("trim"),{8,112,12,116},gworld,draw_rect);
					}
				}
			}
